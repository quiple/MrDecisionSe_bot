...
const keywordList = [
  ...
  { // 할까 (말까)
    keyword: /(?<!가\s|이\s)(?<!\S)([^\s될뭘을일]+까)(?:\S?\??$|\s?말까\S?\??$)/,
    behavior: 'pickOne',
    parameter: ['(아/어)', '지 마', '(아/어)라 좀', '지 말지', '자', '지 말자', '(아/어)라', '지 말어', ('려고?', '으려고?'), '(아/어)야겠냐?'],
  },
  { // 했을까
  ...
  { // 이(가) 할까
    keyword: /(?:가|이)\s([^\s될뭘을일]+까)\S?\??$/,
    behavior: 'pickOne',
    parameter: ['(아/어)', '진 못함', 'ㅁ', '진 못하겠지', 'ㄴ다', '진 못할듯', 'ㄹ듯', '지 못할거 같다', 'ㄹ거 같음?', '겠냐?'],
  },
  { // 인가/일까
    keyword: /([^\s뭐]+(?:인가|일까))\S?\??$/,
    behavior: 'boolean',
...
const behaviors = {
  ...
  pickOne: function (m, list) {
    ...
    temp.q = m.input
    if (m[1]) {
      if (a.analyze(m[1].slice(-8))[0]) {
        res = m[1].slice(0, -8) + a.analyze(m[1].slice(-8))[0][0]._(res)
      } else if (a.analyze(m[1].slice(-7))[0]) {
        res = m[1].slice(0, -7) + a.analyze(m[1].slice(-7))[0][0]._(res)
      } else if (a.analyze(m[1].slice(-6))[0]) {
        res = m[1].slice(0, -6) + a.analyze(m[1].slice(-6))[0][0]._(res)
      } else if (a.analyze(m[1].slice(-5))[0]) {
